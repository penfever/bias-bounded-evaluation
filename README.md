...
Supported CLI estimators: `factor_analysis`, `empirical`, and `domain_specific`. For the richer Combined A‑BB or conformal estimators, prefer driving the Python API or the analysis scripts below.
...
2. **Run debiasing analysis** – use `scripts/analysis/run_combined_abb_analysis.py` (or the Python API) to apply Combined A‑BB, psychometric, or conformal estimators to your score tables.
...
- `differential_debiasing/sensitivity` – individual sensitivity estimators (factor analysis, psychometric reliability, schematic adherence, A‑BB / Combined A‑BB, conformal, etc.).
